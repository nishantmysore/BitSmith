...
async function createPeripheralsInBatches(
  deviceId: string,
  peripherals: any[],
) {
      ...
      batch.map(async (peripheral) => {
        try {
          ...
          if (peripheral.registers?.length) {
            ...
            await createRegistersInBatches(
              createdPeripheral.id,
              peripheral.registers,
            );
          }
        } catch (error) {
          console.error(
            `Failed to create peripheral ${peripheral.name}:`,
            error,
          );
          throw new Error(
            `Failed to create peripheral ${peripheral.name}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }),
    );
    console.log(
      `[Performance] Processed batch of ${batch.length} peripherals in ${Date.now() - batchStartTime}ms`,
    );
  }

  logTiming(
    `Created ${peripherals.length} peripherals with ${totalRegisters} registers`,
    startTime,
  );
}

/* eslint-disable @typescript-eslint/no-explicit-any */
async function createRegistersInBatches(
  peripheralId: string,
  registers: any[],
) {
  const startTime = Date.now();
    ...
    await Promise.all(
      batch.map(async (register) => {
        try {
          ...
          throw new Error(
            `Failed to create register ${register.name}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }),
    );
    console.log(
      `[Performance] Processed batch of ${batch.length} registers in ${Date.now() - batchStartTime}ms`,
    );
  }

  logTiming(
    `Created ${registers.length} registers with ${totalFields} fields`,
    startTime,
  );
}
      ...
      batch.map(async (field) => {
        try {
          ...
          if (field.enumeratedValues?.length) {
            ...
            await createEnumValuesInBatches(
              createdField.id,
          ...
          throw new Error(
            `Failed to create field ${field.name}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }),
    );
    console.log(
      `[Performance] Processed batch of ${batch.length} fields in ${Date.now() - batchStartTime}ms`,
    );
  }

  logTiming(
    `Created ${fields.length} fields with ${totalEnumValues} enum values`,
    startTime,
  );
}
...
async function createEnumValuesInBatches(fieldId: string, enumValues: any[]) {
  ...
  for (let i = 0; i < enumValues.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.log(
        `[Performance] Processed batch of ${batch.length} enum values in ${Date.now() - batchStartTime}ms`,
      );
    } catch (error) {
      console.error(`Failed to create enum values batch:`, error);
      throw new Error(
        `Failed to create enum values: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
      ...
      return NextResponse.json(
        {
          ...
          message:
            "Maximum 100 devices allowed per user. Please delete some devices before creating new ones.",
      ...
      return NextResponse.json(
        {
          error: "Invalid JSON",
          message:
            "The request body could not be parsed as valid JSON. Please check the request format.",
        },
        { status: 400 },
      );
    }
