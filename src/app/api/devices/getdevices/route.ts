...
export async function GET() {
  ...
  console.log(`[Devices API] Prisma connection took ${performance.now() - prismaConnectStart}ms`);
  ...
  try {
    ...
    console.log(`[Devices API] Session fetch took ${performance.now() - sessionStart}ms`);
    ...
    const devices = await prisma.device.findMany({
      ...
      // Add caching hint to Prisma
      cacheStrategy: { ttl: 60 * 60 * 1000 }, // 1 hour cache
    });
    console.log(`[Devices API] Prisma query took ${performance.now() - prismaStart}ms`);
    console.log(`[Devices API] Total request time: ${performance.now() - startTime}ms`);
