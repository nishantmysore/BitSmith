...
import { normalizeHexAddress, parseBitRange, doRangesOverlap } from "@/utils/validation";
...
export const FieldValidateSchema: ZodType<FieldFormData> = z.object({
    ...
    bits: z.string().regex(/^\d+(?::\d+)?$/, "Field bits must be a single number or range"),
...
export const RegisterValidateSchema: ZodType<RegisterFormData> = z.object({
    ...
    address: z.string().regex(/^(0x)?[0-9A-Fa-f]+$/, "Register Address must be a valid hex value"),
...
export const DeviceValidateSchema: ZodType<DeviceFormData> = z.object({
    ...
    base_address: z.string().regex(/^(0x)?[0-9A-Fa-f]+$/, "Device base address be a valid hex value"),
